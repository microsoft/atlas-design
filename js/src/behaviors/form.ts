...
const thereAreNoEditsToSubmit = 'There are no edits to submit';
		...
		try {
			...
			for (const { name, value } of this.attributes) {
				if (name.startsWith('header-')) {
					headers.set(name.substr(7), value);
				}
			}
...
function createErrorAlert(form: HTMLFormElement) {
	...
	errorAlert.className = 'alert is-danger';
	...
	alertText.className = 'alert-title';
...
function createErrorNote(input: HTMLValueElement) {
	...
	note.classList.add('help', 'has-text-danger');
...
function setValidationMessage(element: HTMLValueElement, message: string) {
	...
	const note = group.querySelector('.help.has-text-danger') || createErrorNote(element);
...
function validateMinLength(input: HTMLValueElement, label: string): string | null {
	if (
...
function validateTagSelector(input: HTMLInputElement, label: string): string | null {
	if (input instanceof HTMLInputElement && input.classList.contains('tag-input')) {
		...
		if (!tagCount || tagCount < Number(min) || tagCount > Number(max)) {
			return `${loc_youMustSelectBetweenMinAndMaxTags
				.replace('{min}', min)
				.replace('{max}', max)
...
function showNoChangesMessage(form: HTMLFormElement) {
	...
	errorText.innerText = loc_formBehavior_thereAreNoEditsToSubmit;
